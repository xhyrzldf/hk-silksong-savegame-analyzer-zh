...
const CATEGORY_NAME = "Spool Fragments";
...
export function SpoolFragmentsTab({ parsedJson, decrypted }: TabRenderProps) {
  if (!decrypted || !parsedJson) {
    return <div className="text-white text-center">Load a save file to view spool fragments data.</div>;
  }
...
export function getSpoolFragmentsExtra({ parsedJson, decrypted }: { parsedJson: unknown; decrypted: boolean }): ReactNode {
  if (!decrypted || !parsedJson) {
    return null;
  }
